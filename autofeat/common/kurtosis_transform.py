...
class KurtosisTransform(Transform):
    ...
    def __call__(self, signal_window: np.ndarray, fischer: Union[bool, np.bool_], where: Callable[[Union[int, float, np.int_, np.float_]], Union[bool, np.bool_]] = lambda x: not np.isnan(x)) -> Union[np.float_, np.int_]:
        """
        ...
        """
        return kurtosis_tf(signal_window, fischer=fischer, where=where)
